...
void Options::show_usage( std::string name ) {
    std::string msg;
    if ( name.size() == 0 ) {
        msg = "Usage:\n";
    }
...
// process_argv(). read through all inpuitted args and map them 
// to functionality.
std::unordered_map<std::string,std::string> Options::process_argv(
    int argc,
    char* argv[] ) {
...
//    std::cout << "process_argv(): entering the for loop\n";
    ...
    for ( int i = 1 ; i < argc ; i++ )
    {
        ...
        Options::Option_Switch( argc, argv, idx, ptr );
...
*/
void Options::Option_Switch( int count, char** arg, int* j, std::unordered_map<std::string,std::string>* n )
{
    ...
    if ( std::strcmp(arg[ *j ],"-r") == 0 ) {
        ...
        if ( *j+1 >= count ) {
            ...
            // That's an error.
            std::cout << "\"-r\" option requires letters, exiting (79).\n";
            Options::show_usage("");
            exit(79);
        }
        ...
        if ( isOption( arg[ *j+1 ] ) ) {
            ...
            std::cout << "\"-r\" option requires letters, exiting (78).\n";
            Options::show_usage("");
            exit(78);
        }
        ...
        *j += 1;
        ...
        if ( isOption( arg[ *j+1 ] ) ) {
            ...
            std::cout << "Option_Switch(): \"-x\" option requires a regex, exiting (76).\n";
            Options::show_usage("");
            exit(76);
        }
        ...
        *j += 1;
        ...
        // unrecognized option
        std::cout << "Arg : " << arg[ *j ] << " is not recognized (76).\n";
        Options::show_usage("");
        exit(76);
    }
    // no return value
...
void Letters_In( std::unordered_map<std::string,std::string>* n ) {
    ...
    if ( ( *n ).empty() ) {
        std::cout << "letters_In(): zero options defined, exiting (75).\n";
        exit(75);
    }
    ...
    */
    ...
    if (auto search = ( *n ).find("rank"); search != ( *n ).end()) {
